...
async def test_daily_brief_uses_real_prompt():
    scenario = Scenario.model_validate(
        {
            ...
            "input": {
                "hours": 24,
                "metrics": {
                    "time_in_range_pct": 68.5,
                    "average_glucose": 158.0,
                    "low_count": 2,
                    "high_count": 9,
                    "readings_count": 288,
                    "correction_count": 4,
    ...
    result = await run_scenario(
        scenario, MockClient(content="TIR 68.5% — discuss with your endo.")
    )
...
async def test_correction_uses_real_prompt():
                ...
                "time_periods": [
                    {
                        ...
                        "under_count": 1,
                        "over_count": 4,
                        "avg_observed_isf": 42.0,
                        "avg_glucose_drop": 78.0,
                    }
                ],
            },
            "ground_truth": {"expected_safety_status": "APPROVED"},
    ...
    result = await run_scenario(
        scenario,
        MockClient(content="Evening corrections look strong; discuss with your endo."),
    )
...
async def test_chat_uses_real_web_prompt():
    scenario = Scenario.model_validate(
        {
            ...
            "units": "mg/dL",
            "input": {
                "message": "Why am I high every morning?",
    ...
    result = await run_scenario(
        scenario,
        MockClient(content="Could be dawn phenomenon; discuss with your endo."),
    )
...
async def test_chat_rag_includes_retrieved_knowledge_block():
    ...
    result = await run_scenario(
        scenario,
        MockClient(
            content="It is an early-morning glucose rise; discuss with your endo."
        ),
    )
