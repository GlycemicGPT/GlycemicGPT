...
def _run(latency=1.2, out_tokens=40):
    return RunResult(
        scenario_id="meal-001",
        surface="meal_analysis",
        system_prompt="sys",
        user_prompt="usr",
        output="Breakfast 187 mg/dL.",
        model="mock-model",
        latency_s=latency,
        input_tokens=100,
...
def test_build_report_marks_overall_safety():
    runs = [_run()]
    verdicts = [
        aggregate_verdict("meal-001", [CheckResult("safety", True, False, "ok")])
    ]
...
def test_render_markdown_uses_noncertificate_verdict_vocabulary():
    ...
    # A clean result must read as a SCREEN, never a bare "PASS" certificate.
    assert "Safety screen:" in md
    assert "NOT FLAGGED" in md
    assert "not a medical-safety guarantee" in md.lower()
    assert "mock-model" in md
    assert "2.5" in md


def test_render_markdown_flags_failed_verdict():
    runs = [_run()]
    verdicts = [
        aggregate_verdict(
            "meal-001", [CheckResult("dose_numbers", False, True, "dose")]
        )
    ]
    report = build_report("mock-model", runs, verdicts)
...
def test_render_markdown_marks_unevaluated_output_as_error():
    # No checks ran at all -> ERROR (could not evaluate), gated as NOT safe.
    runs = [_run()]
    verdicts = [aggregate_verdict("meal-001", [])]
    report = build_report("mock-model", runs, verdicts)
    assert report["overall_safety_passed"] is False
