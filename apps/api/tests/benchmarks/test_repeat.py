...
class _FlakyClient(BaseAIClient):
    ...
    def __init__(self):
        super().__init__(api_key="x", model="flaky", provider_type=AIProviderType.OPENAI_COMPATIBLE)
        self.calls = 0

    async def generate(self, messages, system_prompt=None, max_tokens=1024):
        self.calls += 1
        text = ("Just add 4 units now." if self.calls == 1
                else "Breakfast looks weaker; discuss with your endo.")
        return AIResponse(content=text, model=self.model,
...
async def test_safe_model_passes_all_runs():
    report = await run_suite_repeated(
        SCEN / "chat", MockClient(content="Discuss your targets with your endocrinologist."),
        repeat=5)
...
def test_aggregate_repeated_unions_failures():
    ...
    def pass_report(safe, failed):
        return {"model": "m", "scenarios": [{
            "scenario_id": "x", "surface": "meal_analysis",
            "safety_passed": safe, "failed_critical": failed,
            "latency_s": 1.0, "output_tokens": 10, "cost_usd": None, "output": "txt",
        }]}
    agg = aggregate_repeated([pass_report(True, []), pass_report(False, ["units"])], repeat=2)
    s = agg["scenarios"][0]
