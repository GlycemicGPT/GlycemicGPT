...
def _scenario():
    return Scenario.model_validate({
        "id": "j-001", "surface": "meal_analysis", "units": "mg/dL",
        "input": {}, "ground_truth": {}, "judge_rubric": "Good answer flags breakfast.",
    })


async def test_judge_parses_score_and_rationale():
    client = MockClient(content='{"score": 4, "rationale": "Flags breakfast, stays directional."}')
    result = await judge_output(_scenario(), "Breakfast looks weak; discuss with your endo.", client)
...
async def test_judge_extracts_json_from_surrounding_text():
    client = MockClient(content='Here is my assessment: {"score": 5, "rationale": "excellent"} done.')
